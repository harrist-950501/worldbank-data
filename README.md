...
* [CSGO API](https://bymykel.github.io/CSGO-API/)
...
Your website must somehow dynamically load information from the web API you've chosen and present information from that response on the page. This requires that you must:
  * Make at least **two** (2) `fetch` requests in response to 2 distinct UI events that are not the `load` event (in other words, both requests must be in response to an event other than `load`).
    * **Each** fetch request must be in response to a UI event on a **different element** (in other words, it will not count if you make two fetch requests when one button is clicked). The same event can be used (for example, two clicks) but each should be attached to a different element on your page (for example, two different buttons).
    * The `fetch` requests must either be:
      * fetch requests from at least **two** (2) different APIs (in other words, you must make at least 2 requests, each from a completely distinct/different API)
      * OR
      * fetch requests from the **same** API but two **different endpoints**.
        * An API endpoint represents the address to a specific function or resource of the service. Endpoints often (but not always) take parameters. The requests will not be considered distinct if you are making a request to the same endpoint but only changing the parameter in the URL. In other words, you must make a request for at least two completely different resources/data from an API. If you have questions as what counts as distinct endpoint, post on Ed with a reference to both the documentation and a list of the endpoints you're unsure about.
  ...
  * Use the `statusCheck` function from lecture/section to throw an Error if either fetch response status is not `ok` before processing the data. This is a helper function we are requiring you to use (with JSDoc) in your programs, but the rest of your functions must be your own.
  * Handle any errors caused in either fetch request/response process by displaying a helpful message to the user on the page (i.e. without using `alert`, `console.log` or `console.error`). To do so, you should define a function (e.g. `handleError`) to implement the error-message-displaying.
